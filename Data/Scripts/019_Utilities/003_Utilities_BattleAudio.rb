...
def pbGetTrainerBattleBGM(trainer)   # can be a Player, NPCTrainer or an array of them
  ...
  trainerarray.each do |t|
    ...
    return pbStringToAudioFile("CtcUNOwen") if [:Dragon_Dev, :Upside_Down_Dev].include?(trainer_type_data.id) && trainerarray.length() == 1
    return pbStringToAudioFile("ShadHewDuel") if trainer_type_data.id == :Skeleton_Dev
    return pbStringToAudioFile("Miku") if [:CREATOR_Minecraft, :CREATOR_Minecraft2, :COOLTRAINER_MIKU, :Sound_of_Future].include?(trainer_type_data.id)
    ...
    return pbStringToAudioFile("CynthiaEncounter4") if trainer_type_data.id == :TOWER_TYCOON
  end
  ret = pbStringToAudioFile(music) if music && music!=""
  if !ret
    # Check map metadata
