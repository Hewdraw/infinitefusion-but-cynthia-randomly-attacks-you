...
BattleHandlers::MoveImmunityTargetAbility.add(:WONDERGUARD,
  proc { |ability,target,user,move,type,battle|
    next false if ["Fire Fang", "Subzero Ice Fang"].include?(move.name)
...
BattleHandlers::DamageCalcUserAbility.add(:EXPLOSIVE,
  proc { |ability,user,target,move,mults,baseDmg,type|
    mults[:base_damage_multiplier] *= 4 / 3.0 if ["0E0", "170", "356", "360"].include?(move.function)
  }
)
...
BattleHandlers::TargetAbilityOnHit.add(:WEAKARMOR,
  proc { |ability,target,user,move,battle|
    ...
    target.pbRaiseStatStageByAbility(:SPEED, 2, target, false,GameData::Ability.get(ability).real_name)
    battle.pbHideAbilitySplash(target)
  }
)
...
BattleHandlers::TrappingTargetAbility.add(:ARENATRAP,
  proc { |ability,bearer,switcher,battle|
...
BattleHandlers::AbilityOnSwitchIn.add(:TRACE,
  proc { |ability,battler,battle|
    ...
    targets.each do |target|
      ...
      battle.pbDisplay(_INTL("{1} traced {2}'s {3}!",battler.pbThis,target.pbThis(true),target.abilityName))
...
BattleHandlers::AbilityOnSwitchIn.add(:PROTOSYNTHESIS,
  proc { |ability,battler,battle|
    if [:Sun, :HarshSun].include?(battle.pbWeather) && battler.effects[PBEffects::Protosynthesis] == 0
      ...
      stats.each_with_index do |stat,i|
        if stat >= stats.max
          battler.effects[PBEffects::Protosynthesis] = i + 1
          break
        end
      end
      battle.pbShowAbilitySplash(battler)
...
BattleHandlers::AbilityOnSwitchIn.add(:QUARKDRIVE,
  proc { |ability,battler,battle|
    if battle.field.terrain == :Electric && battler.effects[PBEffects::QuarkDrive] == 0
      stageMul = [2, 2, 2, 2, 2, 2, 2, 3, 4, 5, 6, 7, 8]
      stageDiv = [8, 7, 6, 5, 4, 3, 2, 2, 2, 2, 2, 2, 2]
      stats = [:ATTACK, :DEFENSE, :SPECIAL_ATTACK, :SPECIAL_DEFENSE, :SPEED]
      stats2 = [battler.attack, battler.defense, battler.spatk, battler.spdef, battler.speed]
      ...
      stats.each_with_index do |stat,i|
        if stat >= stats.max
          battler.effects[PBEffects::QuarkDrive] = i + 1
          break
        end
      end
      battle.pbShowAbilitySplash(battler)
