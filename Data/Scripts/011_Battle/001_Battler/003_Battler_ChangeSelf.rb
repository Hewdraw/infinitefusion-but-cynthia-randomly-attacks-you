class PokeBattle_Battler
  ...
  def pbRecoverHP(amt,anim=true,anyAnim=true)
    amt *= 0.7 if hasEmera?(:CURSEINCENSE) && !$Trainer.party.include?(@pokemon)
    amt = amt.round
  ...
  def pbFaint(showMessage=true)
    if self.hasActiveAbility?(:SACREDASHES, true) && !@pokemon.battlevariables[:sacredashes]
      ...
      @battle.pbDisplay(_INTL("{1} was sent out!",@battle.battlers[newPkmn].pbThis))
  ...
  def pbPhaseShiftInner(pokemon, splitfrom=nil)
    ...
    pbBGMPlay("VSHo-Oh") if [:LUGIA, :HOOH].include?(pokemon.species)
  ...
  def pbReducePP(move)
    ...
    return true if hasEmera?(:GOLDENLEPPABERRY) && rand(2) == 0
