class PokeBattle_Battler
  ...
  def pbCanInflictStatus?(newStatus,user,showMessages,move=nil,ignoreStatus=false)
    ...
    if self.status==newStatus && !ignoreStatus
      if showMessages
        ...
        when :FROZEN    then msg = _INTL("{1} is already has frostbite!", pbThis)
        end
    ...
    when :FROZEN
      hasImmuneType |= pbHasType?(:ICE) && !hasActiveAbility?([:ICEBODY, :GUTS, :GUTSPLUS])
    end
    if hasImmuneType
      @battle.pbDisplay(_INTL("It doesn't affect {1}...",pbThis(true))) if showMessages
      return false
    ...
    if immuneByAbility
      if showMessages
        ...
        if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
          ...
          when :FROZEN    then msg = _INTL("{1} cannot get frostbite!", pbThis)
          end
        elsif immAlly
          case newStatus
          when :SLEEP
    ...
    if pbOwnSide.effects[PBEffects::Safeguard]>0 && !selfInflicted && move &&
       !(user && user.hasActiveAbility?([:INFILTRATOR, :CHARGEDEXPLOSIVE, :TRUANTPLUS])) && !(user && user.hasActiveAbility?(:VOCALOID) && move.soundMove?)
  ...
  def pbInflictStatus(newStatus,newStatusCount=0,msg=nil,user=nil)
    ...
    user.pbRecoverHP(user.adjustedTotalhp / 10) if user && !user.fainted? && user.hasActiveEmera?(:VANGUARDBELL)
    # Form change check
  ...
  def pbPoison(user = nil, msg = nil, toxic = false)
    if @battle.pbWeather == :Sandstorm || (user && user.hasActiveEmera?(:WITHERSKULL))
      toxic = true
    end
  ...
  def pbCureStatus(showMessages=true)
    ...
    if showMessages
      case oldStatus
      when :SLEEP     then @battle.pbDisplay(_INTL("{1} fought off its drowsiness!", pbThis))
      ...
      when :FROZEN    then @battle.pbDisplay(_INTL("{1}'s frostbite was healed' !", pbThis))
      end
    end
  ...
  def pbAttract(user,msg=nil)
    agender = user.gender
    ogender = gender
    if user.isFusionOf([:FERROSEED, :FERROTHORN]) || isFusionOf([:FERROSEED, :FERROTHORN])
