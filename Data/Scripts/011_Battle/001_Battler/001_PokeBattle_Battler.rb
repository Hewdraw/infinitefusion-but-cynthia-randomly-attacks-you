class PokeBattle_Battler
  ...
  def adjustedTotalhp
    return @totalhp * 2 if @pokemon.dynamax.is_a?(Integer) && @pokemon.dynamax > 0
    return @totalhp * @hpbars if @hpbars
    return @totalhp
  end
  ...
  def hasMoldBreaker?
    return hasActiveAbility?([:MOLDBREAKER, :SCALEBREAKER, :TERAVOLT, :TURBOBLAZE, :TETRAFORCE, :TOTALFREEZE, :CHARGEDEXPLOSIVE, :OBLITERATE])
  end
  ...
  def hasLevitate?
    return true if hasActiveAbility?([:LEVITATE, :EELEVATE, :ENDER, :EONBOOST, :LEVITATIONGUARD, :LEVITATIONAURA, :AERILATEPLUS])
    return false
  end
  ...
  def takesSandstormDamage?
    ...
    return false if hasActiveAbility?([:OVERCOAT, :SANDFORCE, :SANDRUSH, :SANDVEIL, :SANDSTREAM, :ADAPTINGSANDS, :PIXELATEDSANDS, :POWERFULSANDS])
  ...
  def takesHailDamage?
    ...
    return false if inTwoTurnAttack?("0CA", "0CB") # Dig, Dive
    return false if hasActiveAbility?([:OVERCOAT, :ICEBODY, :SNOWCLOAK, :SLUSHRUSH, :SNOWWARNING, :SNOWWWARNING, :ICEFACE])
    return false if hasActiveItem?(:SAFETYGOGGLES)
    return true
  end
