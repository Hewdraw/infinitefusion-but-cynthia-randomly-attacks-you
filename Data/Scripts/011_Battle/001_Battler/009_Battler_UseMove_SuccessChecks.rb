class PokeBattle_Battler
  ...
  def pbTryUseMove(choice,move,specialUsage,skipAccuracyCheck)
    ...
    if @status == :SLEEP && !hasActiveAbility?([:TRUEEYE, :GUTSPLUS])
    ...
    if hasActiveAbility?([:TRUANT, :TRUANTPLUS])
      ...
      if !@effects[PBEffects::Truant]   # True means loafing, but was just inverted
        ...
        if hasActiveAbility?(:TRUANTPLUS)
          pbUseMoveSimple(:SLACKOFF)
        else
          @lastMoveFailed = true
        end
    ...
    # Paralysis
    if @status == :PARALYSIS
      if @battle.pbRandom(8)==0 && !hasActiveAbility?([:QUICKFEET, :GUTSPLUS])
        pbContinueStatus
        @lastMoveFailed = true
        return false
      end
  ...
  def pbSuccessCheckAgainstTarget(move,user,target)
    ...
    if @battle.field.terrain == :Psychic && target.affectedByTerrain? && target.opposes?(user) && @battle.choices[user.index] &&
