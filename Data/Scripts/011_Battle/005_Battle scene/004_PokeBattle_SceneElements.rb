...
class PokemonDataBox < SpriteWrapper
  ...
  def initializeOtherGraphics(viewport)
    ...
    @contents = BitmapWrapper.new(@databoxBitmap.width + 14, @databoxBitmap.height + 50)
  ...
  def refresh
    ...
    if @battler.is_a?(PokeBattle_Battler) && @battler.ability_id == :DEATH
      return
    end
    textPos = []
    imagePos = []
  ...
  def refreshHP
    ...
    return if !@battler.pokemon || (@battler.is_a?(PokeBattle_Battler) && @battler.ability_id == :DEATH)
    # Show HP numbers
    if @showHP
    ...
    # Resize HP bar
    accountedhp = @hpBarSprites.length * @battler.totalhp
    while accountedhp < self.hp
      addHpBar(@viewport)
      accountedhp = @hpBarSprites.length * @battler.totalhp
    end
    @hpBarSprites.each_with_index do |hpbar,i|
  ...
  def updatePositions(frameCounter)
    self.x = @spriteX
    self.y = @spriteY
    self.x += 1000 if (@battler.is_a?(PokeBattle_Battler) && @battler.ability_id == :DEATH)
