...
class PokemonDataBox < SpriteWrapper
  ...
  def refresh
    ...
    if @battler.is_a?(Battler) && @battler.ability_id == :DEATH
      return
    end
    textPos = []
    imagePos = []
  ...
  def refreshHP
    ...
    return if !@battler.pokemon || (@battler.is_a?(Battler) && @battler.ability_id == :DEATH)
    # Show HP numbers
    if @showHP
  ...
  def updatePositions(frameCounter)
    self.x = @spriteX
    self.y = @spriteY
    self.x += 1000 if (@battler.is_a?(Battler) && @battler.ability_id == :DEATH)
