class PokeBattle_Battle
  ...
  def pbSwitchInBetween(idxBattler, checkLaxOnly = false, canCancel = false)
    return pbPartyScreen(idxBattler, checkLaxOnly, canCancel) if pbOwnedByPlayer?(idxBattler) && !($PokemonSystem.aicontrolplayer == 1 && @opponent)
    ...
    loop do
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler) # Opponent/ally is switching in
          next if (wildBattle? || legendaryBattle?) && opposes?(idxBattler) # Wild Pokémon can't switch
          ...
          if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0) == 1 &&
            ...
            switchMessage = $game_switches[SWITCH_GAME_DIFFICULTY_HARD] || pbCynthiaGetBadgeCount >= 8 ? switchMessageHard : switchMessageNormal
  ...
  def pbRecallAndReplace(idxBattler, idxParty, randomReplacement = false, batonPass = false)
    if @battlers[idxBattler].effects[PBEffects::Dynamax] > 0 && !@battlers[idxBattler].fainted?
