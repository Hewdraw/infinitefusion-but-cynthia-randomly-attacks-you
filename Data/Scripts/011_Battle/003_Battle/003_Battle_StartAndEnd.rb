class PokeBattle_Battle
    ...
    loop do
      ...
      for side in 0...2   # Each side in turn
        ...
        sideCounts.each_with_index do |_count,i|
          ...
          # end
          next if sideCounts[i] && requireds[i]<=sideCounts[i]   # Trainer has enough Pokémon to fill their positions
          if requireds[i]==1
  ...
  def pbStartBattleCore
    ...
    when :Snow        then pbDisplay(_INTL("Snow is falling."))
  ...
  def pbGainMoney
    ...
    if trainerBattle? && !legendaryBattle?
      ...
      tMoney *= 2 if @field.effects[PBEffects::RainbowWing]
    ...
    if @field.effects[PBEffects::PayDay]>0
      ...
      @field.effects[PBEffects::PayDay] *= 2 if @field.effects[PBEffects::RainbowWing]
    ...
    if @field.effects[PBEffects::RainbowWing] && trainerBattle? && rand(2) == 0
  ...
  def pbEndOfBattle
    ...
    when 2, 5
      ...
      if @internalBattle
        ...
        if trainerBattle? && @opponent[0].full_name == "Non Skeleton Dev Hewdraw" && opponent.length == 2
