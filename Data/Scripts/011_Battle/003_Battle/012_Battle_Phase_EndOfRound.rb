class PokeBattle_Battle
  ...
  def pbEORWeather(priority)
    ...
    if @field.weatherDuration==0
      ...
      when :Snow      then pbDisplay(_INTL("The snow stopped."))
...
#    when :Snow        then pbDisplay("The snow continues to fall.")
...
#    when :StrongWinds then pbDisplay("The wind is strong.")
  ...
  def pbEndOfRoundPhase(timestop=false)
    ...
    priority.each do |b|
      ...
      if (b.pokemon.affection || b.hasActiveEmera?(:SYNCSTONEULTIMATE)) && rand(100) < 20 && b.status != :NONE && !b.hasActiveAbility?(:BECALMINGBEAUTY)
        oldStatus = b.status
    ...
    # Damage from burn
    priority.each do |b|
      next if b.status != :BURN
      if b.hasActiveAbility?(:WILDFIRE)
        if b.canHeal?
    ...
    priority.each do |b|
      ...
      if idxEncoreMove>=0
        ...
        if b.effects[PBEffects::Encore]==0 || b.moves[idxEncoreMove].pp==0  || b.effects[PBEffects::Dynamax] > 0
