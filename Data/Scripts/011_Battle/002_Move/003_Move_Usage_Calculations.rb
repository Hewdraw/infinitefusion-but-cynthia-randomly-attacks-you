class PokeBattle_Move
  ...
  def pbCalcTypeMod(moveType,user,target,tera=nil)
    ...
    return Effectiveness::NORMAL_EFFECTIVE if moveType == :GROUND &&
       (target.pbHasType?(:FLYING) || (target == tera && target.tera == :FLYING)) && target.hasActiveItem?(:IRONBALL) && @battle.field.effects[PBEffects::InverseRoom] == 0
    # Determine types
    ...
    if moveType == :SHADOW
      ...
      if target == tera && target.tera == :FLYING
        ...
        tTypes.each_with_index do |type,i|
          type = :WATER if type == :GRASS && target.hasActiveItem?(:WELLSPRINGMASK)
          type = :FIRE if type == :GRASS && target.hasActiveItem?(:HEARTHFLAMEMASK)
          type = :ROCK if type == :GRASS && target.hasActiveItem?(:CORNERSTONEMASK)
          typeMods[i] = pbCalcTypeModSingle(moveType,type,user,target)
        end
      end
    end
  ...
  def pbGetAttackStats(user,target)
    ...
    if specialMove?
      return user.attack, user.stages[:ATTACK] if user.hasActiveAbility?(:OBLITERATE)
      return target.spatk, target.stages[:SPECIAL_ATTACK] + 6 if user.hasActiveItem?(:LUCKYPUNCH) && user.isFusionOf([:HAPPINY, :CHANSEY, :BLISSEY])
  ...
  def pbCalcDamageMultipliers(user,target,numTargets,type,baseDmg,multipliers)
    ...
    if user.status == :BURN && physicalMove? && damageReducedByBurn? &&
       !user.hasActiveAbility?([:GUTS, :GUTSPLUS, :WILDFIRE])
      multipliers[:final_damage_multiplier] /= 2.0
    end
    # Frostbite
    if user.status == :FROZEN && specialMove? && !user.hasActiveAbility?([:ICEBODY, :GUTSPLUS])
      multipliers[:final_damage_multiplier] /= 2.0
    end
    # Drowsy
    if target.status == :SLEEP && !(target.pbHasMove?(:SLEEPTALK) || target.pbHasMove?(:SNORE) || target.hasActiveAbility?([:TRUEEYE, :GUTSPLUS]))
    ...
    if protectPenalty? && (target.effects[PBEffects::Protect] || target.effects[PBEffects::KingsShield] || target.effects[PBEffects::Obstruct] || target.effects[PBEffects::SpikyShield] || target.effects[PBEffects::BanefulBunker] || target.effects[PBEffects::BurningBulwark] || target.pbOwnSide.effects[PBEffects::MatBlock])
