...
BattleHandlers::HPHealItem.add(:BERRYJUICE,
  proc { |item,battler,battle,forced|
    ...
    next false if !forced && battler.hp>battler.adjustedTotalhp/2
...
BattleHandlers::AccuracyCalcTargetItem.add(:BUNDLEOFBALLOONS,
  proc { |item,target,mods,user,move,type|
    mods[:accuracy_multiplier] *= 1.2
  }
)
...
BattleHandlers::DamageCalcUserItem.add(:DARKSTONE,
  proc { |item,user,target,move,mults,baseDmg,type|
    mults[:base_damage_multiplier] *= 1.3 if type == :DRAGON
...
BattleHandlers::DamageCalcUserItem.add(:LIGHTSTONE,
  proc { |item,user,target,move,mults,baseDmg,type|
    mults[:base_damage_multiplier] *= 1.3 if type == :DRAGON
    mults[:base_damage_multiplier] *= 1.3 if type == :FIRE
  }
...
BattleHandlers::TargetItemOnHit.add(:BARB,
  proc { |item,target,user,move,battle|
    next if !user.takesIndirectDamage?
    battle.scene.pbDamageAnimation(user)
    user.pbReduceHP(user.totalhp/6,false)
