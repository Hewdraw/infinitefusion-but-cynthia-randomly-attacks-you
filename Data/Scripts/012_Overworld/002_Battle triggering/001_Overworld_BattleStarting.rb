...
def pbPrepareBattle(battle)
  ...
  battle.setBattleMode("2v1") if battle.opponent && ((battle.opponent[0].trainer_type == :WuhuIslandExecutioner && pbCynthiaGetBadgeCount <= 8) || battle.opponent[0].trainer_type == :MECH_Miku) && $Trainer.able_pokemon_count > 1
...
#===============================================================================
def pbLegendaryBattle(species, alpha=false, number=0)
  # Skip battle if the player has no able Pokémon, or if holding Ctrl in Debug mode
  if $Trainer.able_pokemon_count == 0 || ($DEBUG && Input.press?(Input::CTRL))
  ...
  if alpha
    trainer = pbLoadTrainer(:ALPHA_POKEMON, species, number)
...
def pbAfterBattle(decision, canLose)
  if $PokemonGlobal.battledepth != nil && $PokemonGlobal.battledepth > 0
    ...
    $Trainer.party.each do |pkmn|
      $PokemonGlobal.battlehplist[pkmn] = pkmn.hp
    end
    return
  end
  $Trainer.party.each do |pkmn|
