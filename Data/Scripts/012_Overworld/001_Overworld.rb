...
def pbBattleOnStepTaken(repel_active, guarantee=false)
  ...
  if $PokemonGlobal.cynthiachance != nil && pbCynthiaGetBadgeCount >= 3 && $PokemonGlobal.towervalues.nil?
    ...
    if rand(90000000) < $PokemonGlobal.cynthiafieldchance
      $PokemonGlobal.cynthiafieldchance = 0
      $PokemonGlobal.nextBattleBGM = "VSCynthia2"
      pbEncounterCynthia([:CHAMPION_Sinnoh, "Cynthia"], nil, false, 2)
      return
    end
  end
  return if !$PokemonEncounters.encounter_possible_here?
  #pbLegendaryBattle("Meloetta")
...
def healiesTimerCheck
  return if !$PokemonGlobal.healiestimer
